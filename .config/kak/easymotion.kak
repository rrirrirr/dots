...
set-face global EasyMotionForeground rgb:f2a111,rgb:282828+fg
set-face global EasyMotionSelected rgb:000000

try %{
    decl range-specs em_fg
    decl str em_jumpchars atenishrolgdhrkjum
...
#softwrap option
# def easy-motion-w -params 0..2 %{ easy-motion-on-regex '[\t\s][a-zA-Z0-9](?=\n)|([a-zA-Z0-9](?!=\n)[a-zA-Z0-9]+)' 'bGl' %arg{1} %arg{2} }
...
#softwrap option
# def easy-motion-b -params 0..2 %{ easy-motion-on-regex '[\t\s][a-zA-Z0-9](?=\n)|([a-zA-Z0-9](?!=\n)[a-zA-Z0-9]+)' 't' %arg{1} %arg{2} }
...
pydef 'easy-motion-on-selections -params ..3' '%opt{em_jumpchars}^%val{timestamp}^%arg{1}^%arg{2}^%arg{3}^%val{selections_desc}' %{
    ...
    for chars, desc in zip(cs, descs):
        a, h = desc.split(",") #a = start of word | h end of word
        l, c = a.split(".") # l = line, c = column
        ...
        for i in range(1,len(chars)):
            ...
            fgs[chars_i] += " " + q(a + "," + a1 + "|{EasyMotionSelected}" + chars_i) #what to show when one char is entered 1
            fgs[chars_i] += " " + q(a12 + "," + a2 + "|{EasyMotionForeground}" + chars[i:]) #what to show when one char is entered 2
